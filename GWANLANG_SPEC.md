# GwanLang — 나만의 프로그래밍 언어 프로젝트 스펙

> **구현 언어:** Kotlin  
> **언어 유형:** 범용 스크립트 언어 (Lox 스타일)  
> **실행 방식:** Tree-walking Interpreter  
> **참고:** Crafting Interpreters (Robert Nystrom)

---

## 1. 언어 개요

GwanLang은 동적 타입의 스크립트 언어로, 변수, 제어 흐름, 함수, 클로저, 클래스를 지원한다.

```
// Hello World
print "Hello, GwanLang!";

// 변수와 제어 흐름
var name = "World";
if (name == "World") {
  print "Hello, " + name + "!";
}

// 함수와 클로저
fun makeCounter() {
  var count = 0;
  fun increment() {
    count = count + 1;
    return count;
  }
  return increment;
}

var counter = makeCounter();
print counter(); // 1
print counter(); // 2

// 클래스
class Animal {
  init(name) {
    this.name = name;
  }
  speak() {
    print this.name + " makes a sound.";
  }
}

class Dog < Animal {
  speak() {
    print this.name + " barks!";
  }
}
```

---

## 2. 마일스톤 로드맵

### Phase 1: Scanner (Lexer) — 토큰화
**목표:** 소스코드 문자열 → 토큰 리스트 변환

| 항목 | 설명 |
|------|------|
| 입력 | 소스코드 문자열 |
| 출력 | `List<Token>` |
| 핵심 개념 | 상태 머신, 문자 소비, 토큰 분류 |

**지원 토큰:**
- 단일 문자: `( ) { } , . - + ; * /`
- 비교/대입: `! != = == > >= < <=`
- 리터럴: 문자열(`"..."`), 숫자(`123`, `3.14`)
- 식별자 및 키워드: `and, class, else, false, for, fun, if, nil, or, print, return, super, this, true, var, while`

**완료 기준:**
- [x] Token, TokenType 정의
- [x] Scanner 클래스 구현
- [x] 에러 리포팅 (줄 번호 포함)
- [x] 테스트: 모든 토큰 타입 스캔 확인

---

### Phase 2: Parser — 구문 분석
**목표:** 토큰 리스트 → AST(Abstract Syntax Tree) 변환

| 항목 | 설명 |
|------|------|
| 입력 | `List<Token>` |
| 출력 | `List<Stmt>` (AST) |
| 핵심 개념 | Recursive Descent, Precedence Climbing |

**문법 (EBNF):**
```ebnf
program        → declaration* EOF ;
declaration    → varDecl | funDecl | classDecl | statement ;
varDecl        → "var" IDENTIFIER ( "=" expression )? ";" ;
funDecl        → "fun" function ;
function       → IDENTIFIER "(" parameters? ")" block ;
classDecl      → "class" IDENTIFIER ( "<" IDENTIFIER )? "{" function* "}" ;

statement      → exprStmt | printStmt | block | ifStmt | whileStmt | forStmt | returnStmt ;
block          → "{" declaration* "}" ;
ifStmt         → "if" "(" expression ")" statement ( "else" statement )? ;
whileStmt      → "while" "(" expression ")" statement ;
forStmt        → "for" "(" ( varDecl | exprStmt | ";" ) expression? ";" expression? ")" statement ;
returnStmt     → "return" expression? ";" ;

expression     → assignment ;
assignment     → ( call "." )? IDENTIFIER "=" assignment | logic_or ;
logic_or       → logic_and ( "or" logic_and )* ;
logic_and      → equality ( "and" equality )* ;
equality       → comparison ( ( "!=" | "==" ) comparison )* ;
comparison     → term ( ( ">" | ">=" | "<" | "<=" ) term )* ;
term           → factor ( ( "-" | "+" ) factor )* ;
factor         → unary ( ( "/" | "*" ) unary )* ;
unary          → ( "!" | "-" ) unary | call ;
call           → primary ( "(" arguments? ")" | "." IDENTIFIER )* ;
primary        → NUMBER | STRING | "true" | "false" | "nil" | "this"
               | "(" expression ")" | IDENTIFIER | "super" "." IDENTIFIER ;
```

**완료 기준:**
- [ ] Expr sealed class 계층 정의
- [ ] Stmt sealed class 계층 정의
- [ ] Parser 클래스 (Recursive Descent)
- [ ] 구문 에러 복구 (synchronize)
- [ ] 테스트: 파싱 + AST 출력 (Pretty Printer)

---

### Phase 3: Evaluator — 표현식 평가
**목표:** AST의 표현식 노드를 평가하여 값을 반환

| 항목 | 설명 |
|------|------|
| 입력 | `Expr` (AST 노드) |
| 출력 | `Any?` (런타임 값) |
| 핵심 개념 | Visitor 패턴 (Kotlin에서는 when 표현식), 타입 검사 |

**타입 시스템 (런타임):**
- `nil` → Kotlin `null`
- `Boolean` → Kotlin `Boolean`
- `Number` → Kotlin `Double`
- `String` → Kotlin `String`

**완료 기준:**
- [x] 산술/비교/논리 연산 평가
- [x] 런타임 타입 에러 처리
- [x] Truthiness 규칙: `nil`, `false` → falsy, 나머지 → truthy
- [x] 문자열 연결 (`+`)
- [x] 테스트: REPL에서 표현식 평가

---

### Phase 4: Statements & State — 문장과 상태
**목표:** 변수 선언, 대입, 스코프, 제어 흐름 구현

**완료 기준:**
- [x] Environment (스코프 체인) 구현
- [x] 변수 선언 / 대입
- [x] print 문
- [x] 블록 스코프 `{ ... }`
- [x] if / else
- [x] while / for 루프
- [x] 테스트: 피보나치 출력 등

---

### Phase 5: Functions & Closures — 함수
**목표:** 일급 함수, 클로저, return 문 구현

**완료 기준:**
- [x] 함수 선언 및 호출
- [x] 매개변수 바인딩
- [x] return 문 (예외 기반 제어 흐름)
- [x] 클로저 (환경 캡처)
- [x] 네이티브 함수 (`clock()` 등)
- [x] 테스트: 재귀, 클로저 카운터

---

### Phase 6: Resolver — 정적 분석
**목표:** 변수 바인딩을 정적으로 해석하여 성능/정확성 개선

**완료 기준:**
- [x] Resolver 클래스 (AST 순회)
- [x] 변수 사용 전 선언 검증
- [x] 같은 스코프 중복 선언 에러
- [x] 클로저 변수 거리 계산
- [x] 테스트: 스코프 관련 엣지 케이스

---

### Phase 7: Classes — 클래스와 상속
**목표:** 클래스 선언, 인스턴스, 메서드, 상속 구현

**완료 기준:**
- [x] 클래스 선언 및 인스턴스 생성
- [x] 프로퍼티 get/set
- [x] 메서드와 `this` 바인딩
- [x] `init()` 생성자
- [x] 단일 상속 (`<`)
- [x] `super` 키워드
- [x] 테스트: 상속 체인, 메서드 오버라이드

---

### Phase 8: 연산자 확장 — 모듈로 & 복합 대입
**목표:** 산술 연산자 `%`와 복합 대입 연산자 추가

| 항목 | 설명 |
|------|------|
| 입력 | `%`, `+=`, `-=`, `*=`, `/=`, `%=` 토큰 |
| 출력 | 평가된 값 |
| 핵심 개념 | Scanner 토큰 추가, Parser 우선순위, Interpreter 평가 |

**추가 토큰:** `%`, `+=`, `-=`, `*=`, `/=`, `%=`

**완료 기준:**
- [ ] `%` 토큰 및 산술 연산 구현
- [ ] 복합 대입 연산자 (`+=`, `-=`, `*=`, `/=`, `%=`) 구현
- [ ] 0으로 모듈로 연산 시 런타임 에러
- [ ] 테스트: 모듈로 연산, 복합 대입, 에러 케이스

---

### Phase 9: break / continue — 루프 제어
**목표:** while, for 루프에서 흐름을 제어하는 break/continue 문 구현

| 항목 | 설명 |
|------|------|
| 입력 | `break`, `continue` 키워드 |
| 출력 | 루프 흐름 변경 |
| 핵심 개념 | 예외 기반 제어 흐름 (`Return` 패턴 재활용) |

**추가 키워드:** `break`, `continue`

**완료 기준:**
- [ ] `break` 문 — 가장 가까운 루프 즉시 종료
- [ ] `continue` 문 — 현재 반복 건너뛰고 다음 반복으로
- [ ] 루프 밖에서 사용 시 정적 에러 (Resolver 검출)
- [ ] 중첩 루프에서의 동작 검증
- [ ] 테스트: while/for + break/continue 조합

---

### Phase 10: 내장 함수 확장
**목표:** 범용 내장 함수 추가로 기본 유틸리티 제공

| 항목 | 설명 |
|------|------|
| 입력 | 함수 호출 |
| 출력 | 변환/조회 결과 |
| 핵심 개념 | 네이티브 함수 등록 (기존 `clock()` 패턴 확장) |

**추가 내장 함수:**
- `type(value)` → `"number"`, `"string"`, `"boolean"`, `"nil"`, `"function"`, `"class"`, `"instance"`
- `toString(value)` → 문자열 변환
- `toNumber(value)` → 숫자 변환 (실패 시 런타임 에러)
- `input()` → 표준 입력에서 한 줄 읽기
- `println(value)` → 개행 포함 출력

**완료 기준:**
- [ ] `type()` 함수 — 런타임 타입 문자열 반환
- [ ] `toString()` / `toNumber()` 타입 변환 함수
- [ ] `input()` 표준 입력 읽기
- [ ] `println()` 출력 함수
- [ ] 테스트: 모든 타입에 대한 변환, 엣지 케이스

---

### Phase 11: 배열 — 기본
**목표:** 배열 리터럴, 인덱스 접근, 인덱스 대입 구현

| 항목 | 설명 |
|------|------|
| 입력 | `[1, 2, 3]`, `arr[0]`, `arr[0] = 10` |
| 출력 | 배열 값, 인덱스 값 |
| 핵심 개념 | 새 AST 노드, 런타임 타입 `GwanArray` |

**추가 AST 노드:** `Expr.Array`, `Expr.IndexGet`, `Expr.IndexSet`

**완료 기준:**
- [ ] 배열 리터럴 `[expr, expr, ...]` 파싱 및 평가
- [ ] 인덱스 접근 `arr[index]`
- [ ] 인덱스 대입 `arr[index] = value`
- [ ] 범위 초과 인덱스 접근 시 런타임 에러
- [ ] 중첩 배열 `[[1, 2], [3, 4]]`
- [ ] 테스트: 리터럴, 접근, 대입, 중첩, 에러 케이스

---

### Phase 12: 배열 — 메서드 & 순회
**목표:** 배열 내장 메서드와 for-in 순회 구문 추가

| 항목 | 설명 |
|------|------|
| 선행 | Phase 11 (배열 기본) |
| 입력 | 메서드 호출, for-in 구문 |
| 출력 | 변경된 배열 / 순회 실행 |
| 핵심 개념 | 메서드 디스패치, 새 반복 구문 |

**내장 메서드:** `length()`, `push(value)`, `pop()`, `slice(start, end)`
**새 구문:** `for (var x in arr) { ... }`

**완료 기준:**
- [ ] `length()` — 배열 길이 반환
- [ ] `push(value)` — 끝에 요소 추가
- [ ] `pop()` — 마지막 요소 제거 및 반환
- [ ] `slice(start, end)` — 부분 배열 반환
- [ ] `for-in` 순회 구문
- [ ] 테스트: 메서드 조합, 빈 배열, for-in 순회

---

### Phase 13: 문자열 메서드
**목표:** 문자열을 메서드 호출 가능한 객체로 확장

| 항목 | 설명 |
|------|------|
| 입력 | 문자열 메서드 호출, 인덱싱 |
| 출력 | 변환된 문자열/값 |
| 핵심 개념 | 문자열 메서드 디스패치 (Phase 12 패턴 재활용) |

**내장 메서드:** `length()`, `substring(start, end)`, `indexOf(str)`, `split(delimiter)`, `trim()`, `toUpper()`, `toLower()`
**인덱싱:** `str[0]` → 단일 문자 문자열

**완료 기준:**
- [ ] 문자열 인덱싱 `str[index]`
- [ ] `length()`, `substring()`, `indexOf()`
- [ ] `split()`, `trim()`, `toUpper()`, `toLower()`
- [ ] 범위 초과 인덱스 시 런타임 에러
- [ ] 테스트: 각 메서드, 체이닝, 빈 문자열, 에러 케이스

---

### Phase 14: 익명 함수 (람다)
**목표:** 이름 없는 함수 표현식 지원

| 항목 | 설명 |
|------|------|
| 입력 | `fun(params) { body }` 표현식 |
| 출력 | `GwanFunction` 객체 |
| 핵심 개념 | Parser에서 `fun` 키워드를 표현식 위치에서도 파싱 |

**완료 기준:**
- [ ] 익명 함수 표현식 파싱 — `fun(x, y) { return x + y; }`
- [ ] 변수 대입 — `var add = fun(x, y) { return x + y; };`
- [ ] 콜백 전달 — `doSomething(fun(x) { print x; });`
- [ ] 클로저 동작 유지
- [ ] Resolver 정적 분석 호환
- [ ] 테스트: 대입, 즉시 호출, 콜백, 클로저

---

### Phase 15: 문자열 보간
**목표:** 문자열 내부에서 표현식을 삽입하는 보간 구문 지원

| 항목 | 설명 |
|------|------|
| 입력 | `"Hello, ${expression}!"` |
| 출력 | 보간된 문자열 |
| 핵심 개념 | Scanner에서 `${}` 감지, 문자열 연결로 AST 변환 |

**완료 기준:**
- [ ] `${expression}` 기본 보간
- [ ] 중첩 표현식 — `"${1 + 2}"`, `"${obj.method()}"`
- [ ] 보간 없는 기존 문자열과 호환
- [ ] `$` 이스케이프 처리
- [ ] 테스트: 변수, 표현식, 중첩, 이스케이프, 에러 케이스

---

### Phase 16: 에러 핸들링 — try / catch / throw
**목표:** 런타임 에러를 프로그램 내에서 잡고 처리하는 구조적 에러 핸들링

| 항목 | 설명 |
|------|------|
| 입력 | `try { ... } catch (e) { ... }`, `throw expr` |
| 출력 | 에러 포착 및 처리 |
| 핵심 개념 | 새 키워드 3개, Stmt 노드, 런타임 예외 래핑 |

**추가 키워드:** `try`, `catch`, `throw`

**완료 기준:**
- [ ] `throw` 문 — 임의 값을 에러로 던지기
- [ ] `try / catch` 문 — 에러 포착 및 변수 바인딩
- [ ] 잡히지 않은 throw → 기존 런타임 에러 동작 유지
- [ ] 내장 런타임 에러도 catch 가능
- [ ] Resolver 호환 (catch 변수 스코프)
- [ ] 테스트: throw/catch, 중첩, 런타임 에러 포착, 미포착 에러

---

### Phase 17: 맵 (딕셔너리)
**목표:** 키-값 저장소 자료구조 구현

| 항목 | 설명 |
|------|------|
| 선행 | Phase 11 (인덱스 접근 인프라 재활용) |
| 입력 | `{"key": value}`, `map["key"]` |
| 출력 | 맵 값 |
| 핵심 개념 | 런타임 타입 `GwanMap`, 키 타입 제한 (string, number, boolean) |

**내장 메서드:** `keys()`, `values()`, `length()`, `containsKey(key)`, `remove(key)`

**완료 기준:**
- [ ] 맵 리터럴 `{key: value, ...}` 파싱 및 평가
- [ ] 인덱스 접근 `map[key]` / 대입 `map[key] = value`
- [ ] 내장 메서드 (`keys`, `values`, `length`, `containsKey`, `remove`)
- [ ] 존재하지 않는 키 접근 시 `nil` 반환
- [ ] 테스트: 리터럴, 접근, 대입, 메서드, 엣지 케이스

---

### Phase 18: 표준 라이브러리
**목표:** 수학 함수, 파일 I/O 등 실용적인 내장 함수 세트 제공

| 항목 | 설명 |
|------|------|
| 선행 | Phase 16 (에러 핸들링 — I/O 에러 처리) |
| 입력 | 함수 호출 |
| 출력 | 계산/I/O 결과 |
| 핵심 개념 | 네이티브 함수 대량 등록, 에러 처리 통합 |

**수학 함수:** `sqrt(n)`, `abs(n)`, `floor(n)`, `ceil(n)`, `random()`, `pow(base, exp)`
**파일 I/O:** `readFile(path)`, `writeFile(path, content)`, `fileExists(path)`

**완료 기준:**
- [ ] 수학 함수 구현 및 인자 검증
- [ ] 파일 I/O 함수 (읽기/쓰기/존재 확인)
- [ ] I/O 에러 시 catch 가능한 에러 throw
- [ ] 테스트: 수학 함수 정확성, 파일 읽기/쓰기, 에러 케이스

---

### Phase 19: 모듈 시스템 — import
**목표:** 여러 `.gwan` 파일을 분리하여 작성하고 불러오는 모듈 시스템

| 항목 | 설명 |
|------|------|
| 선행 | Phase 18 (표준 라이브러리 안정화 후) |
| 입력 | `import "path.gwan";` |
| 출력 | 모듈 내 선언이 현재 스코프에 바인딩 |
| 핵심 개념 | 파일 로딩, 모듈 스코프 분리, 순환 참조 방지 캐싱 |

**추가 키워드:** `import`

**완료 기준:**
- [ ] `import "file.gwan"` — 파일 로딩 및 실행
- [ ] 모듈별 독립 Environment (스코프 분리)
- [ ] 순환 참조 방지 (이미 로드된 모듈 캐싱)
- [ ] 상대 경로 해석
- [ ] 파일 미발견 시 런타임 에러
- [ ] 테스트: 단일 import, 다중 import, 순환 참조, 에러 케이스

---

## 3. 프로젝트 구조

```
gwanlang/
├── build.gradle.kts
├── src/
│   ├── main/kotlin/gwanlang/
│   │   ├── GwanLang.kt         # 진입점 (REPL + 파일 실행)
│   │   ├── Token.kt            # Token, TokenType
│   │   ├── Scanner.kt          # Phase 1: Lexer
│   │   ├── Expr.kt             # Expression AST 노드
│   │   ├── Stmt.kt             # Statement AST 노드
│   │   ├── Parser.kt           # Phase 2: Parser
│   │   ├── Interpreter.kt      # Phase 3-5: Tree-walking Interpreter
│   │   ├── Environment.kt      # Phase 4: 스코프/변수 관리
│   │   ├── Resolver.kt         # Phase 6: 정적 분석
│   │   ├── GwanFunction.kt     # Phase 5: 함수 객체
│   │   ├── GwanClass.kt        # Phase 7: 클래스 객체
│   │   ├── GwanInstance.kt     # Phase 7: 인스턴스 객체
│   │   └── RuntimeError.kt     # 런타임 에러
│   └── test/kotlin/gwanlang/
│       ├── ScannerTest.kt
│       ├── ParserTest.kt
│       ├── InterpreterTest.kt
│       └── testdata/           # .gwan 테스트 스크립트
│           ├── expressions.gwan
│           ├── scoping.gwan
│           ├── closures.gwan
│           └── classes.gwan
└── docs/
    ├── SPEC.md                 # 이 문서
    ├── GRAMMAR.md              # 문법 정의 (EBNF)
    └── CHANGELOG.md            # 구현 진행 로그
```

---

## 4. 핵심 Kotlin 설계 패턴

### AST 노드 — sealed class + data class

```kotlin
sealed class Expr {
    data class Binary(val left: Expr, val op: Token, val right: Expr) : Expr()
    data class Grouping(val expression: Expr) : Expr()
    data class Literal(val value: Any?) : Expr()
    data class Unary(val op: Token, val right: Expr) : Expr()
    data class Variable(val name: Token) : Expr()
    data class Assign(val name: Token, val value: Expr) : Expr()
    data class Logical(val left: Expr, val op: Token, val right: Expr) : Expr()
    data class Call(val callee: Expr, val paren: Token, val arguments: List<Expr>) : Expr()
    data class Get(val obj: Expr, val name: Token) : Expr()
    data class Set(val obj: Expr, val name: Token, val value: Expr) : Expr()
    data class This(val keyword: Token) : Expr()
    data class Super(val keyword: Token, val method: Token) : Expr()
}
```

### 평가 — when 표현식

```kotlin
fun evaluate(expr: Expr): Any? = when (expr) {
    is Expr.Literal  -> expr.value
    is Expr.Grouping -> evaluate(expr.expression)
    is Expr.Unary    -> {
        val right = evaluate(expr.right)
        when (expr.op.type) {
            TokenType.MINUS -> -(right as Double)
            TokenType.BANG  -> !isTruthy(right)
            else -> null
        }
    }
    is Expr.Binary   -> evaluateBinary(expr)
    // ... 나머지 케이스
}
```

### Environment — 스코프 체인

```kotlin
class Environment(val enclosing: Environment? = null) {
    private val values = mutableMapOf<String, Any?>()

    fun define(name: String, value: Any?) {
        values[name] = value
    }

    fun get(name: Token): Any? {
        values[name.lexeme]?.let { return it }
        enclosing?.let { return it.get(name) }
        throw RuntimeError(name, "Undefined variable '${name.lexeme}'.")
    }

    fun assign(name: Token, value: Any?) {
        if (name.lexeme in values) { values[name.lexeme] = value; return }
        enclosing?.assign(name, value) ?: throw RuntimeError(name, "Undefined variable '${name.lexeme}'.")
    }
}
```

---

## 5. 학습 참고 자료

| 자료 | 설명 |
|------|------|
| [Crafting Interpreters](https://craftinginterpreters.com) | Lox 언어 전체 구현 (Java → C). 이 프로젝트의 주 참고서 |
| [Writing An Interpreter In Go](https://interpreterbook.com) | Go로 Monkey 언어 구현. 다른 시각 제공 |
| [Build Your Own Lisp](http://buildyourownlisp.com) | C로 Lisp 구현. 더 간단한 문법 |
| [PL/0 Wikipedia](https://en.wikipedia.org/wiki/PL/0) | 컴파일러 교육용 미니 언어 참고 |

---

## 6. 진행 추적

| Phase | 상태 | 시작일 | 완료일 | 비고 |
|-------|------|--------|--------|------|
| 1. Scanner | ✅ 완료 | 2026-04-11 | 2026-04-11 | TDD 사이클 12개, 테스트 26건 |
| 2. Parser | ✅ 완료 | 2026-04-19 | 2026-04-19 | 표현식 전용, TDD 사이클 12개 |
| 3. Evaluator | ✅ 완료 | 2026-04-20 | 2026-04-20 | TDD 사이클 14개 |
| 4. Statements | ✅ 완료 | 2026-04-21 | 2026-04-21 | TDD 사이클 27개 |
| 5. Functions | ✅ 완료 | 2026-04-27 | 2026-04-27 | TDD 사이클 22개 |
| 6. Resolver | ✅ 완료 | 2026-04-29 | 2026-04-30 | TDD 사이클 15개 |
| 7. Classes | ✅ 완료 | 2026-05-03 | 2026-05-03 | TDD 사이클 27개 |
| 8. 연산자 확장 | ✅ 완료 | 2026-05-05 | 2026-05-05 | TDD 사이클 20개, 디슈가링 방식 |
| 9. break/continue | ✅ 완료 | 2026-05-05 | 2026-05-05 | Stmt.For 도입, TDD 사이클 17개 |
| 10. 내장 함수 확장 | ⬜ 대기 | | | `type`, `toString`, `toNumber`, `input`, `println` |
| 11. 배열 기본 | ⬜ 대기 | | | 리터럴, 인덱스 접근/대입 |
| 12. 배열 메서드 | ⬜ 대기 | | | `push`, `pop`, `length`, `for-in` |
| 13. 문자열 메서드 | ⬜ 대기 | | | `length`, `substring`, `indexOf`, `split` |
| 14. 익명 함수 | ⬜ 대기 | | | 람다 표현식 |
| 15. 문자열 보간 | ⬜ 대기 | | | `"${expr}"` |
| 16. 에러 핸들링 | ⬜ 대기 | | | `try/catch/throw` |
| 17. 맵 | ⬜ 대기 | | | `{key: value}`, `keys`, `values` |
| 18. 표준 라이브러리 | ⬜ 대기 | | | 수학 함수, 파일 I/O |
| 19. 모듈 시스템 | ⬜ 대기 | | | `import`, 스코프 분리 |
